...
#all_data = pd.concat((train_data.ix[:,1:-1], test_data.ix[:,1:-1]))

def group_data(data, degree=3, threshold = 3):
    new_data = []
    m,n = data.shape
...
def everything(train='data/train.csv', test='data/test.csv', seed = 41, N = 10, data = None, degree=4, threshold=3):
    ...
    submissionFile = 'lr_rare_events_degree=%d_N=%d_seed=%d_threshold=%d_groupby.csv' % (degree, N, seed, threshold)
